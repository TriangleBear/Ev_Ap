...
class RFIDApp:
    ...
    def create_table_window(self, selected_table):
        self.table_window = CTk.CTkToplevel()
        self.table_window.title(f"Event: {selected_table}")
        self.table_window.geometry('600x600')  # Set fixed window size
        self.table_window.attributes('-topmost', False)
        ...
        search_entry = CTk.CTkEntry(self.table_window)
        search_entry.pack(padx=10, pady=10, fill='x')
        search_entry.bind('<KeyRelease>', search_table)

        scrollable_frame = CTk.CTkScrollableFrame(self.table_window)
        ...
        rfid_entry = CTk.CTkEntry(self.table_window)
        rfid_entry.pack(padx=10, pady=10, fill='x')
        rfid_entry.bind('<Return>', lambda event: self.rfid_scan_event(rfid_entry, self.table_window, selected_table, display_data, data))
        ...
        button_frame = CTk.CTkFrame(self.table_window)
    ...
    def redeem_points_button_clicked(self):
        ...
        points_entry = CTk.CTkEntry(redeem_window, placeholder_text="Enter points for this event")
    ...
    def rfid_scan_event(self, entry_widget, table_window, selected_table, display_data, data):
        ...
        if rfid_num in self.rfid_cache:
            last_scan_time = self.rfid_cache[rfid_num]
        ...
        self.rfid_cache[rfid_num] = current_time
        insert_digit(0)
